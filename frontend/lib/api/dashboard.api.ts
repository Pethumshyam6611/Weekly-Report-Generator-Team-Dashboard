...
export const getDashboardSummary = async (params: { week?: string; projectId?: number | string } = {}) => {
  const response = await axiosClient.get<ApiSuccess<DashboardSummary>>('/dashboard/summary', { params });
  return response.data.data;
};

export const getSubmissionStatus = async (params: { week?: string; projectId?: number | string } = {}) => {
  const response = await axiosClient.get<ApiSuccess<SubmissionStatusResponse>>('/dashboard/submission-status', { params });
  return response.data.data;
};

export const getTasksTrend = async (params: { userId?: number | string; projectId?: number | string; startDate?: string; endDate?: string } = {}) => {
...
export const getWorkloadDistribution = async (params: { week?: string; projectId?: number | string } = {}) => {
...
export const getRecentActivity = async (params: { page?: number; perPage?: number; projectId?: number | string } = {}) => {
